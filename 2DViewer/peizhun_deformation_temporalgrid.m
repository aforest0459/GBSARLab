...
% %clear all;
% clc;
function [] = peizhun_deformation_temporalgrid(fullname,pclfile,my_position,rangegrid,fangweigrid)
%新疆紫金数据数据
...
minzz = min(min(zz));
%==========================================================================
%% 打网格xx间隔为每隔10个数据打一个，yy间隔为每隔100个数据打一个
for i = 1:10
zz(i:round(str2double(rangegrid)/0.3):length(yy),:) = minzz;

end
zz(:,1:round(str2double(fangweigrid)/0.0044/750):length(xx))  = minzz;

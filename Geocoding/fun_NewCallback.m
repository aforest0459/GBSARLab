...
% if size(outputSave,2)~=num-1
%     clear global outputSave
% end

%outputSave(num-1,:) = [num-1, ind,pos(1),pos(2),pos(3), m, n, X Y];
output = [num-1, ind,pos(1),pos(2),pos(3), m, n, X Y];
fid = fopen(['selectPointsResult',num2str(num-1),'.txt'],'w');
fprintf(fid,'%d,%d,%.4f,%.4f,%.4f, %d,%d,%.6f,%.6f\n',output');
fclose(fid);
%% 显示游标
output_txt = {[num2str(m),',',num2str(n)],[num2str(X),',',num2str(Y)],['P',num2str(num-1),':',num2str(h.CData(ind))]};

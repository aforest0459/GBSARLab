...
% Last Modified by GUIDE v2.5 19-Nov-2018 16:26:18
...
for i = 1:col
   ...
   handles.defcell   = defcell;
    ...
    col = 1;
    %--------------------获得单个文件的对应时刻---------------
    a1 = strfind(fullname,'.');b = strfind(fullname,'\');
    xuhao = fullname(max(b)+1:max(a1)-1);
    xuhao1 = strrep(xuhao,'AmpDatacorrelationData','');
    %---------------------------------------------------------
    zz = a.correlation;
    xx          = a.xaxis;
    yy          = a.yaxis;
...
%查看该组形变数据
data(data==1e-5)=NaN;
figure;h1=imagesc(-xx,yy,data);colorbar;axis xy;xlabel('方位向（rad）');ylabel('距离向（m）');
set(h1,'alphadata',~isnan(data))     % 无形变处覆白色；
%title(strrep(data,'_','\_'));
% 计算点云的方位角
% 校正参数，若三维测量的空间点相对于雷达距离相对于真实值偏小
...
% 方案2 角度匹配如果p_view 超过了雷达最大角度即continue
    if p_view(j,1)>max(xx) || p_view(j,1)<min(xx)
        ...
        %不等间隔情况
        tmpArray = [xx;p_view(j,1)];
        tmpArraySort = sort(tmpArray,'ascend');
        dxx(j) = find(tmpArraySort==p_view(j,1))-1;
        %等间隔情况
        %dxx(j)           = round((p_view(j,1)-xx(1))/(xx(2)-xx(1)))+1;
            if dyy(j)>0 && dyy(j) < size(nxx,1) && dxx(j) >0 && dxx(j) < size(nyy,2)  
                 X(j)             = nxx(dyy(j),dxx(j));
...
% 无形变位置设置为透明
%withoutdef_points = find(pA_3d_fusion(:,end)==1e-5);
h.CData(pA_3d_fusion(:,end)==1e-5)=NaN;
new_color = h.CData;
set(h,'CData',new_color);
%% 数据游标
hd = datacursormode;
...
caxis([-cmean,cmean]); 
% 计算点云的方位角
%由于相位中心测量误差，计算的pA(:,2)比实际的pA（:,2）要短，造成角度偏大
p_view  = atan(pA(:,1)./pA(:,2));
...
% 方案2 角度匹配如果p_view 超过了雷达最大角度即continue
    if p_view(j,1)>max(xx) || p_view(j,1)<min(xx)
        ...
        tmpArray = [xx p_view(j,1)];
        tmpArraySort = sort(tmpArray,'ascend');
        dxx(j) = find(tmpArraySort==p_view(j,1));
        %等间隔情况
        %dxx(j)           = round((p_view(j,1)-xx(1))/(xx(2)-xx(1)))+1;
            if dyy(j)>0 && dyy(j) < size(nxx,1) && dxx(j) >0 && dxx(j) < size(nyy,2)  
                 X(j)             = nxx(dyy(j),dxx(j));
...
for i = 1:col
   ...
   %无形变数据处理或按相干系数阈值处理
   defcell{i}(zzcell{i}<str2double(get(handles.edit1,'String'))) = 1e-8;                                   % 以0.7作为阈值
   %defcell{i}(zzcell{i}<str2double(get(handles.edit1,'String'))) = NaN; 
   defcell{i} = defcell{i}*1e3;
    zz   =  zz + zzcell{i};
    def  =  def+ defcell{i};    
end
    %% 打网格xx间隔为每隔10个数据打一个，yy间隔为每隔10个数据打一个
mindef = min(min(def));
%for ii = 1:10
n_range = length(1:round(3/0.3):length(yy)); %计算距离向大像元网格数
%循环测试------------------------------------------
% i = 1;def(1,:) = mindef/(n_range-1+1);
% i = 2;def(round(3/0.3)*1+1,:) = mindef/(n_range-2+1);
% i = 3;def(round(3/0.3)*2+1,:) = mindef/(n_range-3+1);
%-------------------------------------------------
for i = 1:n_range
%def(1:round(3/0.3):length(yy),:) = mindef/(n_range-i+1);   %3m为步进间隔，注意round(3/0.3)为步进像素间隔数
def(1+round(3/0.3)*(i-1),:)  = mindef/(n_range-i+1);
end
%end
n_cross = length(1:round(max(xx)/0.044):length(xx));%计算方位向大像元网格数
def(:,1:round(max(xx)/0.044):length(xx))  = mindef; % 44mrad为间隔，xx为弧度；
   ...
   title(handles.axes1,'相干系数图');
   colormap(handles.axes1,'hot') ;
   h = imagesc(handles.axes2,xx,yy,def); colorbar(handles.axes2);
   axis(handles.axes2,'xy')  ;xlabel(handles.axes2,'方位向（Rad）');ylabel(handles.axes2,'距离向（m）');
   colormap(handles.axes2,'default') ;
   title(handles.axes2,'形变图');
   set(h,'alphadata',~isnan(def))     % 无形变处覆白色；
   %% 选择了单个数据
else
    load(fullname);
    col = 1;
...
def(:,1:round(max(xx)/0.044):length(xx))  = mindef; % 44mrad为间隔，xx为弧度；
   ...
   colormap(handles.axes2,'default') ;
   title(handles.axes2,'形变图');
   set(h,'alphadata',def~=col*1e-5)     % 无形变处覆白色；
   %% 选择了单个数据
else
    load(fullname);
    col = 1;
...
def(:,1:round(max(xx)/0.044):length(xx))  = mindef; % 44mrad为间隔，xx为弧度；
%%
h = imagesc(handles.axes2,handles.xaxis,handles.yaxis,def);colorbar(handles.axes2);
   axis(handles.axes2,'xy')  ;xlabel(handles.axes2,'方位向（Rad）');ylabel(handles.axes2,'距离向（m）');
   colormap(handles.axes2,'default') ;
   title(handles.axes2,'形变图');
   set(h,'alphadata',def~=1e-5);    %
%handles.def = def;
handles.def3 = def;
guidata(hObject,handles);
...
for i = 1:n_range
%def(1:round(3/0.3):length(yy),:) = mindef/(n_range-i+1);   %3m为步进间隔，注意round(3/0.3)为步进像素间隔数
DA(1+round(3/0.3)*(i-1),:)  = minDA/(n_range-i+1);
end
%end
n_cross = length(1:round(max(xx)/0.044):length(xx));%计算方位向大像元网格数
DA(:,1:round(max(xx)/0.044):length(xx))  = minDA; % 44mrad为间隔，xx为弧度；
%%
imagesc(handles.axes1,handles.xaxis,handles.yaxis,DA);colorbar(handles.axes1);
   axis(handles.axes1,'xy')  ;xlabel(handles.axes1,'方位向（Rad）');ylabel(handles.axes1,'距离向（m）');
   colormap(handles.axes1,flipud(hot)) ;
   title(handles.axes1,'幅度离差图');
%handles.def = def;
...
yy   = handles.yaxis;
xx   = handles.xaxis;
DA(DA>str2double(get(handles.edit2,'String'))) = 1;
    %% 打网格xx间隔为每隔10个数据打一个，yy间隔为每隔10个数据打一个
minDA = min(min(DA));
%for ii = 1:10
n_range = length(1:round(3/0.3):length(yy)); %计算距离向大像元网格数
...
DA(:,1:round(max(xx)/0.044):length(xx))  = minDA; % 44mrad为间隔，xx为弧度；
%%
h=imagesc(handles.axes1,handles.xaxis,handles.yaxis,DA);colorbar(handles.axes1);
   axis(handles.axes1,'xy')  ;xlabel(handles.axes1,'方位向（Rad）');ylabel(handles.axes1,'距离向（m）');
   colormap(handles.axes1,flipud(hot)) ;
   title(handles.axes1,'幅度离差图');
  % set(h,'alphadata',DA~=1)  ;   % 无形变处覆白色；
%handles.def = def;
handles.DA = DA;
guidata(hObject,handles);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
%% def3形变值投射三维点云，且将DA值约束到最小，比如0.01
...
% %% 数据游标
% hd = datacursormode;
% hd.UpdateFcn = @(obj,event_obj) fun_NewCallback(obj,event_obj,h,p_r,zzcell,xuhao,DEM_Result,3);
%--------------------------图像数据导出------------------------------------
text1.DEM_Result = DEM_Result;
text1.p_r        = p_r;
btn = uicontrol('Style', 'pushbutton', 'String', 'DataOutput',...
        'Position', [5 5 120 20],'Backgroundcolor','w');%
set(btn,'Callback', @(x,y)fun_adjustfig(zzcell,text1,18)); % 使用句柄调用回调函数
...
% btn = uicontrol('Style', 'pushbutton', 'String', 'DataOutput',...
%         'Position', [5 5 120 20],'Backgroundcolor','w');%
% set(btn,'Callback', @(x,y)fun_adjustfig(zzcell,text1,19)); % 使用句柄调用回调函数

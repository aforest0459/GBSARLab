...
%% 用于选择大气校正用的稳定点GCP
elseif flag == 3
%% 用于大气校正稳定点选择的cursor callback
% Display the position of the data cursor
% obj          Currently not used (empty)
% event_obj    Handle to event object
% output_txt   Data cursor text string (string or cell array of strings).
% h            图像句柄
% zz           保存各点pA用于计算各点与雷达的距离
% zzcell       过程累积值
% xuhao        时间序列的对应时刻
% DEM_Result   三维与二维对应关系的检索mat
...
suminversedc = 0;
% 反距离循环求和
for i = 1:length(gcp.position)
    pp(i,:) = gcp.position{i};
    suminversedc = suminversedc + 1/norm(pos-pp(i,:));% 计算每个点的权重 
end
kpqc = gcp.kpqc;
kpq = 0;
% 计算空间每个稳定点的权重
for i = 1:length(gcp.position)
 lambda(i) = (1/norm(pos-pp(i,:)))* suminversedc;
   kpq = kpq + lambda(i).*kpqc{i}; 
end
...
%[m,n] = find(zz >h.CData(ind)-0.000001 & zz<h.CData(ind)+0.000001);  % h.表示把图像的参数传递过来，然后使用里面的CData（颜色值）属性
 %
 zzcell = zzcell';[r,c] = size(zzcell{1});zz = zeros(r,c);
for i = 1:length(zzcell)
    a(i)  = zzcell{i,1}(m(1),n(1)); % 曲线图中每一个值都从zzcell中取出
    zz               =  zz + zzcell{i,1}; % 
    %zz(zz==zz(15,15) | zz==-zz(15,15)) = zz(zz==zz(15,15) | zz==-zz(15,15))*1e-6;
